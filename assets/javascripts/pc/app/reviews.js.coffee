class Reviews
  collapse = ($review_content, complete) ->
    ...
    small_height = 68
    big_height = $expanded.outerHeight() + small_height
    $review_content.height(big_height)
    setTimeout (->
      $review_content.addClass("expanding")
      lib.animation.fade_out $expanded, duration: "short", complete: ->
        $expanded.hide()
        lib.animation.css $review_content, {
          from_css: {height: big_height},
          to_css: {height: small_height},
          complete: ->
...
$(document).on "ajax:before", "form.form-review", (e) ->
  ...
  if result && $form.data("alert-requirement")
    ...
    if length < minimum_length
      if !confirm("글자수를 " + minimum_length + "자 이상 작성하셔야만 적립금이 지급됩니다. 그래도 작성하시겠습니까?")
        e.stopImmediatePropagation()
        result = false
        false
